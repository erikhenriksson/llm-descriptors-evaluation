...
def create_edu_index(output_path="edu_doc_ids.json", batch_size=10000, max_docs=None):
    ...
    try:
        # Stream the dataset to extract just the IDs
        ...
        print("Extracting document IDs (using 'id' field)...")

        for doc in tqdm(edu_dataset):
            try:
                # Using the 'id' field as confirmed by you
                doc_ids.add(doc["id"])
                processed += 1
                ...
                if processed % 1000000 == 0:
                    print(
                        f"Processed {processed} documents, found {len(doc_ids)} unique IDs"
                    )
        ...
        print(
            f"Extraction complete. Found {len(doc_ids)} unique document IDs from {processed} documents."
        )
        ...
        print("Creating empty index file as fallback")
        with open(output_path, "w") as f:
            json.dump([], f)
        return output_path
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create an index of FineWeb EDU document IDs"
    )
    parser.add_argument(
        "--output",
        type=str,
        default="edu_doc_ids.json",
    ...
    create_edu_index(
        output_path=args.output, batch_size=args.batch_size, max_docs=args.max_docs
    )
